...
def build_tool_usage_transaction(
    ...
    """None when the request invoked no tools. Date derivation must match the daily
    spend writer's ``startTime.split("T")[0]`` so rollup rows land in the same UTC
    day bucket as LiteLLM_DailyUserSpend."""
    mcp_names = (
        (mcp_namespaced_tool_name.strip(),) if mcp_namespaced_tool_name and mcp_namespaced_tool_name.strip() else ()
    )
    tool_names = tuple(dict.fromkeys(mcp_names + response_tool_call_names(completion_response)))
    if not tool_names:
        return None
    try:
